...
import AnulomaVilomaInterface from './AnulomaVilomaInterface';
...
const TimerVisual: React.FC<TimerVisualProps> = ({ 
  ...
  // --- Check for Anuloma Viloma ---
  const isAnuloma = patternId === 'anuloma-viloma-base';

  if (isAnuloma && mode !== 'stopwatch') {
      return <AnulomaVilomaInterface phase={phase} currentRound={currentRound} />;
  }
  ...
  // --- Legacy Nose Logic for Surya/Chandra (Anuloma now handled by component above) ---
  const isSurya = patternId === 'surya-bhedana';
  const isChandra = patternId === 'chandra-bhedana';
  const isNoseTechnique = isSurya || isChandra;
  let closeLeft = false;
  ...
  return (
    <div className={`relative ${containerSize} flex items-center justify-center flex-shrink-0`}>
        ...
        >
            ...
            {/* Nose Viz (Legacy for Surya/Chandra) */}
